...
limit = False
limit_num = 1000
database = "context_sub.db"
ll_limit = 3 #lower limit
...
def select_context(cur,vec_lsi,index,sentence,log_file_name):
...
def return_comment(sentence,index,log_file_name):
	...
	low = select_context(all_low,vec_lsi,index,sentence.encode("utf-8"),log_file_name) #sentence for log
...
if __name__ == '__main__':
	...
	param = sys.argv
	sub_ID = param[1] #被験者のID
	log_file_name = ("example_log_" + sub_ID + ".txt")
	...
	while(not end_flag):
		...
		[selected_post,selected_comment] = return_comment(sentence,index,log_file_name)
		print ("#####################################")
